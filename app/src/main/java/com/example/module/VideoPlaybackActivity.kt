...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    override fun onPictureInPictureModeChanged(isInPictureInPictureMode: Boolean) {
        ...
        if (!isInPictureInPictureMode) {
            // App is not in PiP mode, restore the video
            restoreVideoState()
        }
    }

    private fun restoreVideoState() {
        val localVideoUrls = videoUrls // Create a local reference

        // Check if videoUrls is not null and the currentVideoIndex is within bounds
        if (localVideoUrls != null && currentVideoIndex >= 0 && currentVideoIndex < localVideoUrls.size) {
            val currentVideoUrl = localVideoUrls[currentVideoIndex]
            if (currentVideoUrl == null || currentVideoUrl.isEmpty()) {
                // Handle the case when the video URL is not available
                return
            }

            // Load the video with the saved URL and position
    ...
    private fun saveCurrentVideoState() {
        // Save the current video URL and position
        val currentVideoUrl = videoUrls?.get(currentVideoIndex)
        videoPosition = videoView.currentPosition
        // You can save these values to SharedPreferences or any other storage method
    }
//////////////////////////////////////////////////////////////////////////////////////////////
